...
def decrypt(key, source, decode=True):
    ...
    padding = decrypted[-1]  # pick the padding value from the end; Python 2.x: ord(data[-1])
    if decrypted[-padding:] != bytes([padding]) * padding:  # Python 2.x: chr(padding) * padding
